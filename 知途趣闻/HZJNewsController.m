...
- (void)viewDidLoad {
    ...
    self.navigationItem.leftBarButtonItem = [[MMDrawerBarButtonItem alloc] initWithTarget:self action:@selector(leftDraw)];
    ...
    [[HZJWeatherManager sharedInstance] fetchWeatherDataWithCity:@"xian" success:^(HZJWeatherModel *dataModel) {
...
- (void)HZJScrollViewDidChangePage:(NSInteger)index{
    if (self.subViewsArray[index] == [NSNumber numberWithBool:0]) {
        if (index == 1) {
            HZJNewAndWeatherView *view = [[HZJNewAndWeatherView alloc] initWithFrame:CGRectMake(self.view.frame.size.width, 0, self.view.frame.size.width, self.viewRect.size.height) style:UITableViewStylePlain];
            view.delegate = self;
            self.subViewsArray[1] = view;
            ...
            [[HZJNewsManager defaultInstance] fetchNewsListWithTyle:self.titleArray2[index] succeed:^(NSMutableArray<HZJNewsItemModel> * _Nonnull newsList) {
                ...
                [view updateViewWithNewsItemModel:newsList];
            } error:^(NSError * _Nullable error) {
